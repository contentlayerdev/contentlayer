...
import { filePathJoin, fs, relative } from '@contentlayer-temp/utils'
import type { E, HasClock, HasConsole } from '@contentlayer-temp/utils/effect'
import { Array, Chunk, Either, OT, pipe, S, T } from '@contentlayer-temp/utils/effect'
...
export const generateDotpkg = ({
  ...
  pipe(
    ...
    T.rightOrFail,
    OT.withSpan('@contentlayer-temp/core/generation:generateDotpkg', { attributes: { verbose } }),
  )
...
const successCallback = (onSuccess: SuccessCallback | undefined) => {
  ...
  return pipe(
    ...
    OT.withSpan('@contentlayer-temp/core/generation:successCallback'),
  )
}
...
> =>
  pipe(
    ...
    OT.withSpan('@contentlayer-temp/core/generation/generate-dotpkg:writeFilesForCache', {
      attributes: {
        targetPath,
...
export { isType } from 'contentlayer-temp/client'
...
import { SourceProvideSchemaErrorJSON, SourceFetchDataErrorJSON } from 'contentlayer-temp/core'
...
import { dynamicBuildMain } from '@contentlayer-temp/core'
...
const deduplicateContentlayerImportsPlugin = (): esbuild.Plugin => ({
  ...
  setup: (build) => {
    const filter = /\@contentlayer-temp\/[a-z-]+/
    ...
    build.onResolve({ filter: /.+/ }, async ({ path, ...args }) => {
      ...
      if (path.match(filter)) {
        ...
        if (result.path.match(/\@contentlayer-temp\/[a-z-]+\/src\//)) {
