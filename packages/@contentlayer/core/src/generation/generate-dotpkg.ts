...
export const generateDotpkg = ({
  ...
  pipe(
    generateDotpkgStream({ config, verbose }),
    S.take(1),
    S.runCollect,
...
const makeDataExportFile = ({
  ...
  if (docDef.isSingleton) {
    ...
    return `\
// ${autogeneratedNote}
export { default as ${dataVariableName} } from './${docDef.name}/${idToFileName(documentId)}.json'${assertStatement}
`
  }
  ...
  const docImports = documentIds
    .map((_) => `import ${makeVariableName(_)} from './${docDef.name}/${idToFileName(_)}.json'${assertStatement}`)
    .join('\n')
