...
import { Chunk, pipe, T, Tagged } from '@contentlayer/utils/effect'
...
export { createContentStore, type ContentStoreTypes } from './content-store.js'
...
export const loadStackbitConfig = (
  options: { dirPath: string } = { dirPath: '' },
): T.Effect<unknown, StackbitError, Stackbit.Config> =>
  pipe(
    T.tryPromiseOrDie(() => Stackbit.loadConfig({ dirPath: options.dirPath })),
    T.chain((configResult) =>
      T.gen(function* ($) {
        if (configResult.errors.length > 0) {
          return yield* $(T.fail(new StackbitError({ errors: configResult.errors })))
        }

        if (configResult.config === null) {
          return yield* $(T.fail(new StackbitError({ errors: [new Error('Config is null') as any] })))
        }

        const stackbitConfig = configResult.config!

        const validatedStackbitConfig = validateStackbitConfig(stackbitConfig)

        return validatedStackbitConfig
      }),
    ),
  )
...
): T.Effect<unknown, StackbitError, SourceFiles.DocumentType[]> =>
  pipe(
    loadStackbitConfig({ dirPath: options.dirPath }),
    T.chain((stackbitConfig) =>
      T.gen(function* ($) {
        if (options.fetchSchema) {
          if (stackbitConfig.contentSources === undefined || stackbitConfig.contentSources.length === 0) {
            return yield* $(T.fail(new StackbitError({ errors: [new Error('No content sources defined') as any] })))
          }

          const models = yield* $(
            pipe(
              T.forEachPar_(stackbitConfig.contentSources, (contentSource) =>
                T.tryPromiseOrDie(async () => {
                  await contentSource.init({
                    logger: createNoopLogger(),
                    userLogger: createNoopLogger(),
                    localDev: true,
                    webhookUrl: undefined,
                    userCommandSpawner: (() => {}) as any,
                    devAppRestartNeeded: () => {},
                    cache: {} as unknown as StackbitTypes.Cache,
                    runCommand: (() => {}) as any,
                    git: {
                      getRepoUrl: () => '',
                      getRepoBranch: () => '',
                  ...
                  const models = (await contentSource.getSchema()).models
                  return models
                }),
              ),
              T.map(Chunk.toArray),
          ...
          return stackbitConfigToDocumentTypes(patchedConfig, overrideArgs)
        } else {
          return stackbitConfigToDocumentTypes(stackbitConfig, overrideArgs)
        }
      }),
    ),
  )

export class StackbitError extends Tagged('StackbitError')<{
  readonly errors:
    | Stackbit.ConfigValidationError[]
    | (Stackbit.ConfigLoadError | Stackbit.StackbitConfigNotFoundError)[]
}> {
  toString = () => `StackbitError: ${this.errors.map((error) => error.message).join('\n')}`
}
...
export const stackbitConfigToDocumentTypes = <TDocumentTypeNames extends core.GetDocumentTypeNamesGen>(
  ...
  const models = stackbitConfig.models.filter(not(isImageModel))
